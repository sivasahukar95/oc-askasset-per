...
logger.info("MilvusWrapper initialized with db_path: %s and collection_name: %s", db_path, collection_name)

# Delete all existing data from the collection
milvus_wrapper.delete_all_data()
logger.info("Deleted all existing data from the collection: %s", collection_name)

# Load data from CSV and insert into Milvus
csv_file_path = "./assethub_oct16th2024.csv"
data = milvus_wrapper.load_data(csv_file_path)
milvus_wrapper.insert_data(data)
logger.info("Loaded data from CSV file: %s and inserted into the collection", csv_file_path)
...
class LLMBackendCloud:
    def __init__(self, model_id='meta-llama/llama-3-70b-instruct', model_params=DEFAULT_LLM_PARAMS):
        ...
        logger.info("LLM Backend initialized with model_id: %s", model_id)

    def generate_response(self, prompt: str, model_params=None):
        """Generate a response from the LLM based on the provided prompt."""
        logger.info("Generating response with prompt: %s", prompt)
        if not model_params:
...
def extract_author_from_query(query):
    ...
    if match:
        ...
        if author_candidate.lower() not in [keyword.lower() for keyword in tech_keywords]:
            logger.info("Extracted author from query: %s", author_candidate)
    ...
    if possessive_match:
        ...
        if author_candidate.lower() not in [keyword.lower() for keyword in tech_keywords]:
            logger.info("Extracted author from possessive query: %s", author_candidate)
            return author_candidate
...
def extract_entities_from_query(query):
    ...
    logger.info("Extracted entities from query: %s", entities)
...
def extract_relevant_keywords(query):
    ...
    logger.info("Relevant keywords extracted: author: %s, entities: %s", author, entities)
    return {
...
def prepare_prompt(user_query_text, search_results):
    ...
    """
    formatted_results = [{"assetId": result['id']} for result in search_results]
    results_str = json.dumps(formatted_results)
    
    logger.info("Prepared prompt for LLM with user query: %s", user_query_text)
    return f"""
...
def map_hashed_ids_to_original(milvus_wrapper, search_results):
    ...
    logger.info("Mapped hashed IDs to original IDs in search results")
...
def clean_results(results):
    ...
    logger.info("Cleaned search results to remove duplicates")
...
def search_by_keywords_or_author(milvus_wrapper, keywords, user_query_text, limit=3):
    ...
    if author:
        ...
        logger.info("Performing vector search for query: %s", user_query_text)
...
def handle_user_query(query_request: QueryRequest):
    ...
    logger.info("Handling user query: %s with limit: %d", user_query_text, limit)
    try:
        ...
        if search_results:
            ...
            logger.info("Query handled successfully, returning results")
            return formatted_results
        else:
            logger.warning("No relevant assets found for the query: %s", user_query_text)
            return {
                "query": user_query_text,
                "message": "No relevant assets found for the given query."
            }
    except Exception as e:
        logger.error("Error occurred while handling the query: %s", str(e))
